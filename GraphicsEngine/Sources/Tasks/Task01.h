...
public:
	...
	virtual void Init()
	{
		...
		Object * pObj1 = NULL;
		{
			...
			pObject->m_pMesh		= new MeshCube(10);//кол-во^2 кубов на одной стороне, из которых будет состоять ОСНОВНОЙ КУБ 
			pObject->m_pTransform	= new Transform();
			pObject->m_pMaterial	= new MaterialUnlitSimple();
			pObject->AddComponent(new ObjectRotator(0, 45, 0));//кручение
		...
		Object * pObj2 = NULL;
		{
			...
			pObject->m_pMaterial	= new MaterialUnlitSimple();
			pObject->AddComponent( new ObjectRotator(0, 90, 0) );//кручение
		...
		// Object #3 - Cube (Moon)
		{
			...
			pObject->m_pMaterial	= new MaterialUnlitSimple();
			pObject->AddComponent( new ObjectRotator(0, 360, 0) );//кручение
